...
static float piTunerAbsFloat(float value)
{
...
static void piTunerApplyTargets(void)
{
    ...
    WHEEL_SPEED_setTargetsMmps(gTargetMmps, gTargetMmps);
}
...
static void piTunerHandleBtPacket(const uint8_t *frame, uint8_t frameLength)
{
    ...
    int16_t targetMmps;
    float targetYawDeg;
    float targetYawDeltaDeg;
    uint8_t run;
    uint8_t targetYawChanged = 0U;
    uint16_t kpX100;
    uint16_t kiX100;
    uint16_t kdX100;
    ...
    targetYawDeltaDeg =
        piTunerAbsFloat(ANGLE_CONTROL_wrapErrorDeg(targetYawDeg, gTargetYawDeg));
    if (targetYawDeltaDeg > 0.001f) {
        gTargetYawDeg = targetYawDeg;
        targetYawChanged = 1U;
    }
    if ((targetYawChanged != 0U) ||
        (ANGLE_CONTROL_isActive() == 0U)) {
        ANGLE_CONTROL_setTargetYawDeg(targetYawDeg);
        WHEEL_SPEED_setTargetsMmps(0, 0);
        TB6612_stopAll();
    }
}
...
void PI_TUNER_init(void)
{
    ...
    gOuterLinePid.p = 0.0f;
    gOuterLinePid.i = 0.0f;
    gOuterLinePid.d = 0.0f;
    gImuInitialized = 0U;
    gImuInitAttempted = 0U;
    piTunerPrintDebugMenu();
}
